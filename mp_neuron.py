...
df1 = train_binarised.iloc[:, :87]
df2 = train_binarised.iloc[:, 87:]
df1.iloc[:, 0] = train_new.iloc[:, 0]
dff1 = df1.to_numpy()
dff2 = df2.to_numpy()
...
class mp_neuron:
    def __init__(self):
        self.b = None
    def model(self, x):
        return (1 if sum(x) - x[0] >= self.b else 0)
    def predict(self, X):
        Y = []
        for x in X:
...
mpn = mp_neuron()
mpn.fit(dff1, dff2)
sub = mpn.predict(test_binarised.to_numpy())
submission = pd.DataFrame({'PhoneId':test_new['PhoneId'], 'Class':sub})
